...
python3 -m defectguard.cli evaluating \
    -model simcom \
    -feature_test_set "dataset/$project/$setup/$setup-$project-features-test.jsonl" \
    -commit_test_set "dataset/$project/$setup/$setup-$project-simcom-test.jsonl" \
    -dictionary "dataset/$project/$setup/dict-$project.jsonl" \
    -dg_save_folder $setup/rus \
    -repo_name $project \
    -device cuda \
    -repo_language C
